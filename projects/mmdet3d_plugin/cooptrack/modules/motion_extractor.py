...
class MotionExtractor(nn.Module):
    def __init__(self,
        ...
        """
        PointNet 구축
        박스의 코드 8개 점을 256 차원으로 embedding
        """
        pointnet = []
        ...
        """
        Geometry + Velocity 정보를 결합하여 motion embedding 생성
        """
        fusion_net = []
    ...
    def forward(self, track_instances, img_metas):
        ...
        decode_bboxes = denormalize_bbox(pred_bboxes, None) # 실제 좌표로 복원
        bboxes_geometric = decode_bboxes[:, 0:7] # box 위치 정보 추출
        bbox = LiDARInstance3DBoxes(bboxes_geometric,
                                    box_dim=7)
        corners = bbox.corners
        center = bbox.gravity_center
        normalized_corners = corners - center.unsqueeze(1) # Corner 좌표를 center 기준으로 정규화 (절대 위치는 빼고, 박스의 모양/방향/크기 정보만 남김)
        # Geometry Embedding
